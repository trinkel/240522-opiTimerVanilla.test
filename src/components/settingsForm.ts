			...
			<form id="settings-form">
				...
				<fieldset id="timing">
					<sl-select id="practice-length" name="practice-length"
						...
						help-text="Practice Length from schedule or 104" value="${parameters.practiceLength.toString()}" clearable
						required>
					...
					<sl-radio-group id="pause-between-selector"
						...
						help-text="Will there be a pause between teams" value="${
							parameters.pauseBetweenSelector
						}">
					...
					<sl-input id="pause-length" name="pause-length" class="label-on-left"
						...
						placeholder="00:00"
						 value="${stringifySeconds(parameters.pauseLength, false, 'colon')}"></sl-input>
				</fieldset>

				<fieldset id="operation-mode">
					<sl-radio-group id="operation-mode-selector"
						name="operation-mode-selector" class="label-on-left"
						label="Select operation mode" value="${parameters.operationMode}" required>
					...
					<sl-input id="number-teams" name="number-teams"
						...
						min="1" step="1" value="${parameters.numberTeams.toString()}" required></sl-input>
					<sl-textarea id="team-list" name="team-list"
						class="label-on-left" label="Team List" value="${parameters.teamList}" disabled
